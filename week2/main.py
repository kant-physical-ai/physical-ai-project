...
def draw_detection(frame: np.ndarray, objects: list[DetectedObject]):
    ...
    for obj in objects:
        ...
        cv2.putText(result_frame, f"({obj.cx}, {obj.cy})", (obj.cx - 10, obj.cy - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    return result_frame
...
def main():
    ...
    def while_video_read(frameData: lib.device.FrameData):
        ...
        cv2.putText(combine_frame, f"FPS: {frameData.fps:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
